...
var DateField = React.createClass({
  componentDidMount: function() {
    ...
    jQuery(this.refs.datepicker).pickadate({
      ...
      onOpen: function() {
        ...
        $wrap.toggleClass(
          'top',
          offset.top + height > window.innerHeight + $(window).scrollTop()
        );
  ...
  onKeyPress: function(e) {
    ...
    let input = this.refs.datepicker.value;
    if (/[^0-9\.]/g.test(e.key)) {
      e.preventDefault();
    }
    if (e.key === '/') {
      if (value.length === 1) this.refs.datepicker.value = `0${value}/`;
      else if (value.length === 2 || value.length === 5)
        this.refs.datepicker.value = `${value}/`;
      else if (value.length === 4)
        this.refs.datepicker.value = `${value.slice(0, 3)}0${value.slice(
          3 + Math.abs(0)
        )}`;
  ...
  onKeyUp: function(e) {
    if (
      e.key !== 'Backspace' &&
      (e.currentTarget.value.length === 2 || e.currentTarget.value.length === 5)
    )
  ...
  toggleCalendar: function(event) {
    ...
    var picker = jQuery(ReactDOM.findDOMNode(this.refs.datepicker)).pickadate(
      'picker'
    );
    if (picker.get('open')) {
      picker.close();
